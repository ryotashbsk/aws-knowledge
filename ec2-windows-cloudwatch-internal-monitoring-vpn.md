...
# Apache / MySQL ポート監視
$ports = @(
    @{ Name = "Apache80PortAlive"; Host = "localhost"; Port = 80 },
    @{ Name = "Apache443PortAlive"; Host = "localhost"; Port = 443 },
    @{ Name = "MySQL3306PortAlive"; Host = "localhost"; Port = 3306 }
)

foreach ($port in $ports) {
    $value = Test-PortAlive -HostName $port.Host -Port $port.Port

    $metrics += @{
        MetricName = $port.Name
        Value = $value
        Unit = "Count"
        Dimensions = @(
            @{ Name = "InstanceId"; Value = $instanceId },
            @{ Name = "Endpoint"; Value = "$($port.Host):$($port.Port)" }
        )
    }
}
...
if ($metrics.Count -gt 0) {
    $json = ConvertTo-Json -InputObject $metrics -Depth 5 -Compress
    $tempFile = Join-Path $env:TEMP "cloudwatch-metrics.json"

    # AWS CLI がBOMを読めないため、BOMなしUTF-8で保存
